...
func (t *DataProtocolSmartContract) Init( stub shim.ChaincodeStubInterface ) pb.Response {
	...
	business_list, _ := json.Marshal( []string{} )
	err2 := stub.PutState( IndexBusinessList, business_list )
	if err2 != nil {
		return shim.Error( "ERROR: INITIALISING THE BUSINESS LIST: " +
	                       err2.Error() ) }
	...
	// Initialise list of guarantors in the Smart Contract as empty //
	guarantor_list, _ := json.Marshal( []string{} )
	err4 := stub.PutState( IndexGuarantorList, guarantor_list )
	if err4 != nil {
		return shim.Error( "ERROR: INITIALISING THE USER LIST: " +
						   err4.Error() ) }
	
	// Initialise list of court members in the Smart Contract as empty //
	court_member_list, _ := json.Marshal( []string{} )
	err5 := stub.PutState( IndexCourtMemberList, court_member_list )
	if err5 != nil {
		return shim.Error( "ERROR: INITIALISING THE USER LIST: " +
						   err5.Error() ) }
						   
	// Initialise list of exchange in the Smart Contract as empty //
	exchange_list, _ := json.Marshal( []string{} )
	err6 := stub.PutState( IndexExchangesList, exchange_list )
	if err6 != nil {
		return shim.Error( "ERROR: INITIALISING THE USER LIST: " +
						   err6.Error() ) }

	// Initialise list of maps for encrypting targeting IDs//
	target_list, _ := json.Marshal( make( map[string]bool ) )
	err7 := stub.PutState( IndexTargetEncryption, target_list )
	if err7 != nil {
		return shim.Error( "ERROR: INITIALISING THE TARGET ENCRYPTION: " +
	                        err7.Error() ) }
...
func (t *DataProtocolSmartContract) register( stub shim.ChaincodeStubInterface,
	...
	if actor.Role != ADMIN_ROLE {
		actor.PublicId = xid.New().String() + xid.New().String() }
	...
	invoke_call := []string{ "registerWallet" }
	...
	// If the actor is a Guarantor, add it to list //
		...
		if err11 != nil {
			return shim.Error( "ERROR: ADDING GUARANTO " + actor.PublicId + 
...
/* -------------------------------------------------------------------------------------------------
getUser:  this function returns the object containing all the information for a PRIVI actor.
          Args: array contaning:
PublicId            string   // Public Id key of the acotr
------------------------------------------------------------------------------------------------- */

func (t *DataProtocolSmartContract) getUser( stub shim.ChaincodeStubInterface,
											 args []string ) pb.Response {

	// Retrieve arguments of the input //
	if len(args) != 1 {
		return shim.Error( "ERROR: GETUSER FUNCTION SHOULD BE CALLED " +
						   "WITH ONE ARGUMENTS." ) }
	publicId := args[0]
